...
public final class ParBoruvka extends AbstractBoruvka<ParBoruvka.ParComponent> {
    ...
    public void computeBoruvka(final Queue<ParComponent> nodesLoaded,
        ...
        while (!nodesLoaded.isEmpty()) {
            ...
            if (current == null || current.isDead || !current.lock.tryLock()) {
                continue;
            }
            ...
            final ParComponent otherComponent = minEdge.getOther(current);
            ...
            current.merge(otherComponent, minEdge.weight());
            current.lock.unlock();
